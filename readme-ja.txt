...
開発版 **v0.5.0-dev** は、v0.4 系の機能に加え、`wpmar_report_sections` / `wpmar_notification_channels` / `wpmar_backup_providers` などの拡張用フックと、既定オフのデータベースサイズチェック（`information_schema`）、通知ディスパッチを追加します。
...
* **出力** — 詳細な **管理者向け** Markdown（アップロード先）と、任意の HTML メール（クライアント向け・管理者向けのペア）。
