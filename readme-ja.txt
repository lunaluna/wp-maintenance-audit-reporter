...
**多層防御:** (1) ファイル名に推測不能なランダムトークンを付与（Web サーバーの種類に依存しない唯一の防御）、(2) 各ディレクトリに `.htaccess`（`Require all denied` / `Deny from all`）と `index.php` を自動生成、(3) 既定でドキュメントルート配下だが後述の定数でドキュメントルート外へ退避可能、(4) マルチサイトでは基底ディレクトリの下に `site-{サイトID}/` を必ず挟んでサイト間を分離（`wp-content` はネットワーク共有のため）。

**保存先の変更** — `wp-config.php` などで `WPMAR_PRIVATE_STORAGE_DIR` を定義するとドキュメントルート外へ退避できます（例: `define( 'WPMAR_PRIVATE_STORAGE_DIR', dirname( ABSPATH ) . '/wpmar-private' );`）。`wpmar_private_storage_dir` フィルターでも同様に上書きできます。

**Apache 以外のサーバー（nginx など）** — `.htaccess` は Apache 専用のため、nginx では `/wp-content/wpmar-private/` に `deny all; return 404;` の明示的な拒否設定が必要です。CDN / リバースプロキシを使う構成では同様の設定を CDN 側にも追加することを推奨します。

**書き込み不可時のフォールバック** — `wp-content` への書き込みができない環境では、保護済みの `wp-content/uploads/wpmar/` へ自動的にフォールバックし、管理画面に通知が表示されます。フォールバック中もランダムトークンによる保護は維持されます。

**バックアップ運用** — `wp-content/wpmar-private/` はバックアップ対象から除外するか、含める場合は暗号化を推奨します（管理者のメールアドレスや脆弱性情報を含むため）。

**v1.3.0 以前からのアップグレード** — 既存の md / PDF / ログファイルは、プラグイン更新後に自動的に新しい保存先へ移行されます（新しいランダムトークンが付与されます）。移行は少しずつバックグラウンドで進行し、管理画面には進捗が表示されます。移行が完了するまで旧ファイルは削除されません。移行中にプロセスが強制終了しても、次回のアクセス時に続きから再開されます。手動で確認・実行する方法は後述の「`wp wpmar storage migrate`」を参照してください。

**ダウングレード時の注意** — v1.3.0 以前の zip に戻す場合は、事前に `wp wpmar storage migrate --revert`（後述）を実行してください。実行せずにダウングレードすると、`wp-content/wpmar-private/` 配下のファイルが retention 削除やアンインストール削除の対象から外れて残置される可能性があります。
...
* **対象サイト**（最大サイト数・除外 blog ID）— 絞るのは**監査そのもの**。保存され、除外したサイトは監査が実行されずスナップショットも更新されません。
* **レポート出力範囲**（v1.5.4〜）— 絞るのは**レポートに掲載するサイトだけ**。保存され、定期実行を含む全実行に適用されます。監査は全サイトで実行され、スナップショットは常に更新されます。掲載範囲だけが変わります。
* **ドライランの対象サイト** — ドライラン（試し実行）だけの一時指定。保存されません。v1.5.4 以降は「今すぐ実行」には影響しません（それ以前は「今すぐ実行」にも適用されていました）。
...
WordPress / 実行環境の目安: **PHP 7.4+**。WordPress **6.0+**、動作確認済みは **7.1**。
...
* 変更：ネットワーク管理画面の「今すぐ実行」は常にすべての対象サイトを監査するようになりました。既存の実行範囲セレクター(すべての対象サイト / 親サイトのみ / 特定のサイトのみ)はドライラン専用になります。「今すぐ実行」を絞り込むと、選ばなかったサイトのスナップショット基準が古いまま残ってしまう問題があったためです。サイトを絞った実行が必要な場合はドライラン、または WP-CLI の `--same-setting` / `--id=<blog_id>` をご利用ください。
* 詳細は CHANGELOG.md を参照してください。
...
* 修正：更新の有無を判定する版比較が、`WPMAR_VERSION` 定数ではなく WordPress が実際に認識しているインストール済みバージョン（プラグインヘッダー由来、`$transient->checked` 経由）を基準にするようになりました。両者がずれても「更新あり」の通知が消えない、または出ないという問題が起きなくなります。
* 修正：更新チェッカーのキャッシュが、`update_plugins` 自体と同じサイト単位のトランジェントになりました（従来はブログ単位）。マルチサイトでサブサイトごとにGitHub APIを問い合わせることがなくなります。
* 修正：更新詳細モーダルに表示される「Requires PHP」「Tested up to」の値が、ハードコードではなくプラグインヘッダーから読み込まれるようになり、ヘッダーとのずれがなくなりました。
* 変更：リリースワークフローのバージョン突合を、従来の1箇所（タグ↔ヘッダー）から5箇所（プラグインヘッダー・`WPMAR_VERSION`・両readmeファイルの `Stable tag`・`composer.json`）に拡張しました。バージョン番号の誤記をリリース前に検出できます。
* 変更：`release.yml` と `bin/build-zip.sh` が、配布ZIPから除外するファイルの一覧を `.distignore` として一本化しました。従来は別々に管理されており実際にずれが生じていました（CIが作るZIPに `bin/` が同梱されていた、フォントを生成済みの環境でローカルビルドすると `fonts/` が混入する場合があった、など）。
* 詳細は CHANGELOG.md を参照してください。
