...
開発版 **v0.3.x** では、v0.2 のチェックサム・レポート機能に加え、TLS・PHP サポート・管理者ログイン状況など運用上のセキュリティ関連シグナルをレポートに含めます。
...
* **管理画面** — トップレベル **Maintenance Audit** メニュー（`admin.php` 画面）。**設定・実行**（スケジュール、メール、除外、保持、実行）と **レポート**（一覧テーブル、1 ページ 20 件、詳細、行単位と一括削除は独自確認ダイアログ。成功通知はトランジェントのフラッシュ表示で、クエリ引数の貼り付きはしない）。
...
* 管理: トップレベル Maintenance Audit メニュー。レポート一覧（ページネーション、削除と一括削除、独自モーダル、トランジェントのフラッシュ通知）。従来の `wpmar_msg` を URL から除去。
