...
**v0.6.0** では、関係者向け・運用者向けメールを改善しました。クライアント向けは **HTML メール**（PDF と同じ Markdown を Parsedown で HTML 化）、件名は保守スクリプト相当の形式、クライアント本文に **公式ディレクトリの最終更新から見た「更新停滞」プラグイン**の節を追加し、**管理者向けメールはナマ JSON ではなく** mainte.sh 風の **整形プレーンテキスト**に変更しました。**v0.5** 由来の拡張（`wpmar_report_sections` / `wpmar_notification_channels` / `wpmar_backup_providers`）、既定オフの DB サイズサンプル、`information_schema`、通知ディスパッチも引き続き利用できます。
...
* **PDF（クライアント向け／任意）** — フル実行のたびに `uploads/wpmar/pdf/*.pdf` を生成可能（**クライアント向け** Markdown をソースとする）。ランタイム依存のためプラグイン直下で `composer install` が必要です。
...
= 0.4.0-dev =
* PDF（クライアント向け）: mPDF/Parsedown によりフル実行時に uploads/wpmar/pdf へ保存（設定で制御）。
