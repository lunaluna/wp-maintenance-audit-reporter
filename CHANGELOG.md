...
- **Schema/repository groundwork for per-site network jobs (not yet wired up)** — New `{$wpdb->prefix}wpmar_network_segments` table (one row per `run_id`+`blog_id`, `queued`→`running`→`done`/`failed`, an `attempts` counter, and only the rendered `client_body`/`admin_body` — never the raw per-site dataset) plus a `WPMAR_Network_Segments_Repository` CRUD façade mirroring `WPMAR_Jobs_Repository`'s design. `wpmar_jobs` also gains an `attempts` column. This is pure schema/repository scaffolding for splitting a network rollup into one independent async job per site (so one site's failure or memory spike can't affect the others or accumulate across the whole run) — the dispatcher, per-site job handler, and aggregate step that will actually use these land in follow-up changes.
