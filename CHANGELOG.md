...
- **Schema/repository/handler groundwork for per-site network jobs (not yet wired up)** — New `{$wpdb->prefix}wpmar_network_segments` table (one row per `run_id`+`blog_id`, `queued`→`running`→`done`/`failed`, an `attempts` counter, and only the rendered `client_body`/`admin_body` — never the raw per-site dataset) plus a `WPMAR_Network_Segments_Repository` CRUD façade mirroring `WPMAR_Jobs_Repository`'s design. `wpmar_jobs` also gains an `attempts` column. `WPMAR_Job_Dispatcher::run_network_site_segment()` (new Action Scheduler hook `wpmar/run_network_site_segment`) now runs a single blog's `WPMAR_Runner::run_site_segment()` as its own action and records the result to the segments table — this is the fix for the structural memory ceiling of the old design (looping every site inside one Action Scheduler action, so peak memory grows with site count and one site's OOM takes the whole run down): splitting each site into its own action caps peak memory at one site's worth regardless of network size, and an OOM here only fails that one segment. `WPMAR_Job_Dispatcher::run_network_aggregate()` (new hook `wpmar/run_network_aggregate`) waits for every segment to reach `done`/`failed` — self-rescheduling every `wpmar_network_aggregate_recheck_delay` seconds (default 3 min, unmeasured) while any are still `queued`/`running`, force-failing a segment stuck `running` past `wpmar_network_segment_stale_minutes` (default 20 min, unmeasured) or every still-incomplete segment once `wpmar_network_aggregate_max_wait` (default 90 min, unmeasured) elapses since dispatch — then finalizes through the same merge/mail/PDF/report-insert/retention/reschedule logic the old single-action design used, now extracted into `WPMAR_Network_Runner::finalize_rollup()` so both this and the old synchronous path share it. A `failed` segment gets a "このサイトはエラーのため取得できませんでした" note in the merged report instead of its (nonexistent) body — a report goes out for every site that finished even if one didn't. Nothing enqueues either hook as part of an actual network run yet — the dispatcher rewrite that does lands in a follow-up change.
