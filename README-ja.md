...
プレビューの展開ボタンの上に、常時表示の「基準の鮮度（最終保存日時）」一覧があります（プレビューを開かなくても見えます）。各項目（core/themes/plugins/users）の最新 `captured_at` が並び、直近の定期実行が実際に差分比較の基準を更新できたかを一目で確認できます。最も古い項目の基準が60日以上前の場合は注意書きが表示されます（60日は未実測の暫定値で、月次運用を前提とした1サイクル分の猶予として置いています）。「いつと比べたか」「今回更新したか」と同じ情報は、レポート本文の「前回スナップショットからの差分」直下にも実行ごとに表示され、`summary_json` にも記録されます。
