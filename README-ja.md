...
WordPress 用プラグイン：コア・テーマ・プラグインの定期保守監査 — **v1.4.0**。
...
- WP-CLI での同期実行（`wp wpmar audit run --sync`）でもログは生成されます（ジョブ ID は `cli-` で始まります）が、管理画面のジョブ一覧には出ないため、サーバー上で直接プライベート保存先の `logs/`（後述）を確認してください。
